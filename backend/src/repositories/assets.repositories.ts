...
import { AssetModel, IAsset } from '../models/assets.model';
...
export class AssetRepository {
  ...
  static async findById(id: string): Promise<IAsset | null> {
    return await AssetModel.findById(id).exec();
  }

  static async findMany(user_id: Schema.Types.ObjectId): Promise<IAsset[]> {
    const assets = await AssetModel.find({ user_id });
  ...
  static async incrementDownloadCount(id: string): Promise<void> {
    await AssetModel.findByIdAndUpdate(id, { $inc: { download_count: 1 } });
  }
